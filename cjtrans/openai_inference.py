...
class OpenaiModelPredictor(object):
    def __init__(
        ...
        if self.model_name == "deepseek-chat" or self.model_name == "deepseek-coder":
            self.tokenizer = transformers.AutoTokenizer.from_pretrained("deepseek-ai/DeepSeek-Coder-V2-Lite-Instruct")
        elif self.model_name == "qwen2.5-72b-instruct":
            self.tokenizer = transformers.AutoTokenizer.from_pretrained("Qwen/Qwen2.5-72B-Instruct")
        elif self.model_name == "yi-large":
            self.tokenizer = transformers.AutoTokenizer.from_pretrained("01-ai/Yi-1.5-34B-Chat")
        elif self.model_name == "chatglm3-6b":
            self.tokenizer = transformers.AutoTokenizer.from_pretrained("THUDM/chatglm3-6b")
